...
##----------------------------------------------------------------------------------
        ...
        self.type_plot_plCh.addItems(['no filter', 'Fourier highpass', 'Fourier lowpass', 'Fourier both', 'Fourier multiple', 'SavGol', 'Binning']) 
        #### plot buttom
        ...
        layout_plCh.addWidget(self.static_canvas_plCh, QtCore.Qt.AlignTop) # align the plot up
        layout_plCh.addStretch() # stretch plot in all free space
        self.toolbar = NavigationToolbar(self.static_canvas_plCh,self.Plot_chs, coordinates=True) # add toolbar below the plot
...
#----------------------------------------------------------------------------------
        ...
        self.type_plot_LOS.addItems(['no filter', 'Fourier highpass', 'Fourier lowpass', 'Fourier both', 'Fourier multiple', 'SavGol', 'Binning']) 
        #### plot buttom
        ...
        layout_LOS.addWidget(self.static_canvas_LOS, QtCore.Qt.AlignTop) # align the plot up
        layout_LOS.addStretch() # stretch plot in all free space
        self.toolbar = NavigationToolbar(self.static_canvas_LOS,self.LOS_plot, coordinates=True) # add toolbar below the plot
...
#----------------------------------------------------------------------------------
        ...
        self.type_plot_FFT.addItems(['no filter', 'Fourier highpass', 'Fourier lowpass', 'Fourier both', 'Fourier multiple', 'SavGol', 'Binning']) 
        #### plot buttom
        ...
        layout_FFT.addWidget(self.static_canvas_FFT, QtCore.Qt.AlignTop) # align the plot up
        layout_FFT.addStretch() # stretch plot in all free space
        self.toolbar = NavigationToolbar(self.static_canvas_FFT,self.FFT_plot, coordinates=True) # add toolbar below the plot
...
##----------------------------------------------------------------------------------
        ...
        self.type_plot_Rtr.addItems(['no filter', 'Fourier highpass', 'Fourier lowpass', 'Fourier both', 'Fourier multiple', 'SavGol', 'Binning']) 
        #### plot buttom
        ...
        layout_Rtr.addWidget(self.static_canvas_Rtr, QtCore.Qt.AlignTop) # align the plot up
        layout_Rtr.addStretch() # stretch plot in all free space
        self.toolbar = NavigationToolbar(self.static_canvas_Rtr,self.R_trace, coordinates=True) # add toolbar below the plot
...
##----------------------------------------------------------------------------------
        ...
        self.type_plot_Rsing.addItems(['no filter', 'Fourier highpass', 'Fourier lowpass', 'Fourier both', 'Fourier multiple', 'SavGol', 'Binning']) 
        ...
        layout_Rsing.addWidget(self.static_canvas_Rsing, QtCore.Qt.AlignTop) # align the plot up
        layout_Rsing.addStretch() # stretch plot in all free space
        self.toolbar = NavigationToolbar(self.static_canvas_Rsing,self.R_single, coordinates=True) # add toolbar below the plot
...
##----------------------------------------------------------------------------------
        ...
        self.type_plot_Ztr.addItems(['no filter', 'Fourier highpass', 'Fourier lowpass', 'Fourier both', 'Fourier multiple', 'SavGol', 'Binning']) 
        #### plot buttom
        ...
        layout_Ztr.addWidget(self.static_canvas_Ztr, QtCore.Qt.AlignTop) # align the plot up
        layout_Ztr.addStretch() # stretch plot in all free space
        self.toolbar = NavigationToolbar(self.static_canvas_Ztr,self.Z_trace, coordinates=True) # add toolbar below the plot
...
##----------------------------------------------------------------------------------
        ...
        self.type_plot_Zsing.addItems(['no filter', 'Fourier highpass', 'Fourier lowpass', 'Fourier both', 'Fourier multiple', 'SavGol', 'Binning']) 
        ...
        layout_Zsing.addWidget(self.static_canvas_Zsing, QtCore.Qt.AlignTop) # align the plot up
        layout_Zsing.addStretch() # stretch plot in all free space
        self.toolbar = NavigationToolbar(self.static_canvas_Zsing,self.Z_single, coordinates=True) # add toolbar below the plot
...
##----------------------------------------------------------------------------------
        ...
        #### plot buttom
        self.Butt_plot_EQHt = QPushButton("Load EQH, CEC, ECEI_fakeRz and Plot",self.EQH_tab)
        ...
        layout_EQHt.addWidget(self.static_canvas_EQHt, QtCore.Qt.AlignTop) # align the plot up
        layout_EQHt.addStretch() # stretch plot in all free space
        self.toolbar = NavigationToolbar(self.static_canvas_EQHt,self.EQH_tab, coordinates=True) # add toolbar below the plot
...
##----------------------------------------------------------------------------------
        ...
        self.ImgType_plot_RzPl.addItems(['no Image filter', 'Gaussian', 'Median', 'Bilateral', 'Conservative_smoothing']) 
        self.type_plot_RzPl = QComboBox(self.Rz_tab) 
        self.type_plot_RzPl.addItems(['no 1D filter', 'Fourier highpass', 'Fourier lowpass', 'Fourier both', 'Fourier multiple', 'SavGol', 'Binning']) 
        self.Interp_plot_RzPl = QComboBox(self.Rz_tab) 
        self.Interp_plot_RzPl.addItems(['no interpolation', 'with interpolation','set to zero']) 
        # self.Interp_plot_RzPl.setMaximumSize(QtCore.QSize(90, 0))
        self.Save_plot_RzPl = QComboBox(self.Rz_tab) 
        self.Save_plot_RzPl.addItems(['do not save', 'save as pdf','save as png']) 
        #### plot buttom
        ...
        layout_RzPl.addWidget(self.static_canvas_RzPl, QtCore.Qt.AlignTop) # align the plot up
        layout_RzPl.addStretch() # stretch plot in all free space
        self.toolbar = NavigationToolbar(self.static_canvas_RzPl,self.Rz_tab, coordinates=True) # add toolbar below the plot
...
##----------------------------------------------------------------------------------
        ##  SettRz tab - content
        ### Create layouts
        layout_RzSet = QtWidgets.QVBoxLayout(self.SettRzPlot_tab) #main layout
        ...
        self.Info1_lbl_RzSet.setText('====== Matrix for interpolation (scipy.interpolate.interp2d, type = cubic) or "set to zero" options ======')
        ...
        for sText, pos in LOSlabels.items():
            ...
            sublayout2_RzSet.addWidget(self.LOSlabels[sText], pos[0]+1, pos[1])
        ...
        for sText, pos in checks.items():
            ...
            sublayout2_RzSet.addWidget(self.checks[sText], pos[0]+1, pos[1]+1)
        ...
        self.path_lbl_RzSet.setText('Path to save Rz plots (path should end with "/" symbol):')
...
##----------------------------------------------------------------------------------
    ...
    def Load_ECEI_data(self):
        try:
            ...
            self.Monitor_load.setText("Status:\nLoading %s: #%d ... "%(self.Diag,self.Shot))
            allow_to_load = True
        ...
        if (self.Diag == 'TDI') & (allow_to_load == True):
            try:
                TD=TDI.TDI()
                TD.Load(self.Shot)
                TD.Load_FakeRz()
                ...
                self.Monitor_load.setText("Status:\nTDI #%d\ntB = %g, tE = %g s\nLoaded succesfully."%(self.Shot, TD.time[0], TD.time[-1]))
        ...
        if (self.Diag == 'ECI') & (allow_to_load == True):
            try:
                EI=ECI.ECI()
                EI.Load(self.Shot)
                EI.Load_FakeRz()
                ...
                self.Monitor_load.setText("Status:\nECI #%d\ntB = %g, tE = %g s\nLoaded succesfully."%(self.Shot, EI.time[0], EI.time[-1]))
                NN_LOS = self.ECEId.shape[1]
                add_to_LOS = np.arange(1,NN_LOS+1)
                add_to_LOS = [str(e) for e in add_to_LOS]
                self.NLOS_type_LOS.clear()
    ...
    def plot_chs_all(self):
        ...
        if (self.data_loaded == True): # check whether ECEI data is loaded
            try:
                ...
                if (self.type_plot_plCh.currentText() == 'Fourier lowpass'):
                    ...
                    noise_ampl = 1.0
                    mf.Fourier_analysis_ECEI_lowpass(time_plot, data_plot, noise_ampl, f_cut)
                    data_plot = mf.ECEId_fft_f_ifft
                    filter_status = "Fourier lowpass, freq_cut = %g kHz"%(f_cut*1.0e-3)

                if (self.type_plot_plCh.currentText() == 'Fourier highpass'):
                    f_cut = float(self.Fourier2_cut_plCh.text())*1.0e3
                    noise_ampl = 1.0
                    mf.Fourier_analysis_ECEI_highpass(time_plot, data_plot, noise_ampl, f_cut)
                    data_plot = mf.ECEId_fft_f_ifft
                    filter_status = "Fourier highpass, freq_cut = %g kHz"%(f_cut*1.0e-3)
                ...
                if (self.type_plot_plCh.currentText() == 'Fourier both'):
                    ...
                    noise_ampl_hp = 1.0
                    mf.Fourier_analysis_ECEI_lowpass(time_plot, data_plot, noise_ampl_lp, f_cut_lp)
                    data_plot = mf.ECEId_fft_f_ifft.copy()
                    mf.Fourier_analysis_ECEI_highpass(time_plot, data_plot, noise_ampl_hp, f_cut_hp)
                    data_plot = mf.ECEId_fft_f_ifft.copy()
                    filter_status = "Fourier high and low pass, freq_cut_hp = %g kHz, freq_cut_lp = %g kHz"%(f_cut_hp*1.0e-3,f_cut_lp*1.0e-3)
                ...
                if (self.type_plot_plCh.currentText() == 'Fourier multiple'):
                    ...
                    for i in range(freq_num):
                        ...
                        f_lp[i] *= 1.0e3 
                    mf.Fourier_analysis_ECEI_multiple(time_plot, data_plot, f_hp, f_lp)
                    data_plot = mf.ECEId_fft_f_ifft
                    filter_status = "Fourier multiple, freqs: %s kHz"%(string)
                ...
                if (self.type_plot_plCh.currentText() == 'SavGol'):
                    ...
                    filter_status = "Savgol, win_len = %g, pol_ord = %g"%(win_len,pol_ord)

                if (self.type_plot_plCh.currentText() == 'Binning'):
                    time_plot, data_plot
                    binning_freq = float(self.Binning_ed_plCh.text())
                    time_plot, data_plot = mf.dataBinningECEI(time_plot, data_plot,binning_freq)
                ...
                self._static_ax = self.static_canvas_plCh.figure.subplots(NN_LOS, NN_R, sharex=True, sharey=False) # add axes

                for L_i in range(NN_LOS):
                    for R_i in range(NN_R):
                        self._static_ax[L_i,R_i].plot(time_plot,data_plot[:,L_i,R_i])
                # draw the figure
                self.figure_plCh.suptitle("ECEI, Shot #%d, deltaTrad/<Trad>, Filter: %s"%(self.Shot,filter_status), fontsize=8)
    ...
    def f_LOS_plot(self):
        ...
        if (self.data_loaded == True): # check whether ECEI data is loaded
            try:
                ...
                self._static_ax = self.static_canvas_LOS.figure.subplots(4, 2, sharex=True, sharey=False) # add axes


                for i_R in range(8):
                    self._static_ax[i_R-4*(i_R//4),i_R//4].plot(time_plot,data_plot[:,LOStoPlot,i_R])
                    self._static_ax[i_R-4*(i_R//4),i_R//4].set_title("R_ch=%g"%(i_R+1))
                    self._static_ax[i_R-4*(i_R//4),i_R//4].set_ylabel("deltaTrad/<Trad>")
                    self._static_ax[i_R-4*(i_R//4),i_R//4].grid()
                    self._static_ax[3,0].set_xlabel("t [s]")
                    self._static_ax[3,1].set_xlabel("t [s]")
                # draw the figure
                self.figure_LOS.suptitle("ECEI, Shot #%d, LOS=%g, Filter: %s"%(self.Shot,LOStoPlot+1,filter_status), fontsize=10)
    ...
    def f_FFT_plot(self):
        ...
        if (self.data_loaded == True): # check whether ECEI data is loaded
            try:
                ...
                self._static_ax = self.static_canvas_FFT.figure.subplots(4, 2, sharex=True, sharey=False) # add axes


                for i_R in range(8):
                    freq_fft, Te_fft, offset = mf.fft_analysis(time_plot,data_plot[:,LOStoPlot,i_R])
                    NN = len(freq_fft)
                    self._static_ax[i_R-4*(i_R//4),i_R//4].plot(freq_fft[:int(NN/2)]*1e-3, np.abs(Te_fft[:int(NN/2)]),"b")
                    self._static_ax[i_R-4*(i_R//4),i_R//4].set_title("R_ch=%g"%(i_R+1))
                    self._static_ax[i_R-4*(i_R//4),i_R//4].set_ylabel("|FFT(deltaT/<T>)|")
                    self._static_ax[i_R-4*(i_R//4),i_R//4].grid()
                    self._static_ax[3,0].set_xlabel("freq [kHz]")
                    self._static_ax[3,1].set_xlabel("freq [kHz]")
                # draw the figure
                self.figure_FFT.suptitle("ECEI, Shot #%d, LOS=%g, Filter: %s"%(self.Shot,LOStoPlot+1,filter_status), fontsize=10)
    ...
    def plot_R_trace(self):
        ...
        if (self.data_loaded == True): # check whether ECEI data is loaded
            try:
                ...
                if (self.vmin_ed_Rtr.text().replace('-','',1).replace('.','',1).isdigit()==True):
                    vmin = float(self.vmin_ed_Rtr.text())
                else:   
                    vmin = None

                if (self.vmax_ed_Rtr.text().replace('.','',1).isdigit()==True):
                    vmax = float(self.vmax_ed_Rtr.text())
                else:   
                    vmax = None

                if (self.NNcont_ed_Rtr.text().replace('.','',1).isdigit()==True):
                ...
                self._static_ax = self.static_canvas_Rtr.figure.subplots(1, len(ch_zz), sharex=False, sharey=True) # add axes

                for i_z in range(len(ch_zz)):
                    contours = self._static_ax[i_z].contourf(R_tr,time_tr,data_plot[:,ch_zz[i_z],ch_RR].T, vmin = vmin, vmax = vmax, levels = NN_cont, cmap='jet')
                    cbar = self.figure_Rtr.colorbar(contours, ax=self._static_ax[i_z], orientation="horizontal", pad = 0.07)
                    if contour_check == '1':
                        self._static_ax[i_z].contour(R_tr,time_tr,data_plot[:,ch_zz[i_z],ch_RR].T, vmin = vmin, vmax = vmax, levels = NN_cont, cmap='binary')
                    cbar.ax.tick_params(labelsize=8, rotation=90) 
                    self._static_ax[i_z].set_xlabel("R [m](LOS=%g)"%(ch_zz[i_z]+1))
                    self._static_ax[0].set_ylabel("t [s]")

                self.figure_Rtr.suptitle("ECEI, Shot #%d, deltaTrad/<Trad>, Filter: %s"%(self.Shot,filter_status), fontsize=10)
    ...
    def plot_R_single(self):
        ...
        if (self.data_loaded == True): # check whether ECEI data is loaded
            try:
                ...
                if removeRR_ch:
                    removeRR_ch = np.array(self.chRR_ed_Rsing.text().split(','))
                    removeRR_ch = removeRR_ch.astype(int) - 1
                else:
                    removeRR_ch = []
                ...
                if (self.vmin_ed_Rsing.text().replace('-','',1).replace('.','',1).isdigit()==True):
                    vmin = float(self.vmin_ed_Rsing.text())
                else:   
                    vmin = None

                if (self.vmax_ed_Rsing.text().replace('.','',1).isdigit()==True):
                    vmax = float(self.vmax_ed_Rsing.text())
                else:   
                    vmax = None

                if (self.NNcont_ed_Rsing.text().replace('.','',1).isdigit()==True):
                ...
                contours = ax.contourf(time_tr, R_tr, data_plot[:,LOStoPlot,ch_RR].T, vmin = vmin, vmax = vmax, levels = NN_cont, cmap='jet')
                cbar = self.figure_Rsing.colorbar(contours, ax=ax)
                cbar.ax.set_ylabel('deltaTrad/<Trad>', rotation=90)
                if contour_check == '1':
                    ax.contour(time_tr, R_tr, data_plot[:,LOStoPlot,ch_RR].T, vmin = vmin, vmax = vmax, levels = NN_cont, cmap='binary')
                cbar.ax.tick_params(labelsize=8, rotation=90) 
                ...
                for i in range(len(ch_RR)):
                    ax.annotate(ch_RR[i]+1, (time_tr[i,0], R_tr[i,0]), size=10)

                self.figure_Rsing.suptitle("ECEI, Shot #%d, Filter: %s"%(self.Shot,filter_status), fontsize=10)
    ...
    def plot_Z_trace(self):
        ...
        if (self.data_loaded == True): # check whether ECEI data is loaded
            try:
                ...
                if (self.vmin_ed_Ztr.text().replace('-','',1).replace('.','',1).isdigit()==True):
                    vmin = float(self.vmin_ed_Ztr.text())
                else:   
                    vmin = None

                if (self.vmax_ed_Ztr.text().replace('.','',1).isdigit()==True):
                    vmax = float(self.vmax_ed_Ztr.text())
                else:   
                    vmax = None
                if (self.NNcont_ed_Ztr.text().replace('.','',1).isdigit()==True):
                ...
                self._static_ax = self.static_canvas_Ztr.figure.subplots(4, 2, sharex=True, sharey=False) # add axes

                for i_R in range(8):
                    contours = self._static_ax[i_R-4*(i_R//4),i_R//4].contourf(time_tr,z_tr,data_plot[:,ch_zz,i_R].T, vmin = vmin, vmax = vmax, levels = NN_cont ,cmap='jet')
                    self.figure_Ztr.colorbar(contours, ax=self._static_ax[i_R-4*(i_R//4),i_R//4])
                    if contour_check == '1':
                        self._static_ax[i_R-4*(i_R//4),i_R//4].contour(time_tr,z_tr,data_plot[:,ch_zz,i_R].T, vmin = vmin, vmax = vmax, levels = NN_cont ,cmap='binary')
                    self._static_ax[i_R-4*(i_R//4),i_R//4].set_title("R_ch=%g"%(i_R+1))
                    self._static_ax[i_R-4*(i_R//4),i_R//4].set_ylabel("z [m]")
                    self._static_ax[3,0].set_xlabel("t [s]")
                    self._static_ax[3,1].set_xlabel("t [s]")
                    my_text = ch_zz
                    for i in range(len(ch_zz)):
                        self._static_ax[i_R-4*(i_R//4),i_R//4].annotate(ch_zz[i]+1, (time_tr[i,0], z_tr[i,0]), size=6)


                self.figure_Ztr.suptitle("ECEI, Shot #%d, deltaTrad/<Trad>, Filter: %s"%(self.Shot,filter_status), fontsize=10)
    ...
    def plot_Z_single(self):
        ...
        if (self.data_loaded == True): # check whether ECEI data is loaded
            try:
                ...
                if removeLOS_ch:
                    removeLOS_ch = np.array(self.chzz_ed_Zsing.text().split(','))
                    removeLOS_ch = removeLOS_ch.astype(int) - 1
                else:
                    removeLOS_ch = []
                ...
                if (self.vmin_ed_Zsing.text().replace('-','',1).replace('.','',1).isdigit()==True):
                    vmin = float(self.vmin_ed_Zsing.text())
                else:   
                    vmin = None

                if (self.vmax_ed_Zsing.text().replace('.','',1).isdigit()==True):
                    vmax = float(self.vmax_ed_Zsing.text())
                else:   
                    vmax = None
                if (self.NNcont_ed_Zsing.text().replace('.','',1).isdigit()==True):
                ...
                contours = ax.contourf(time_tr,z_tr,data_plot[:,ch_zz,RtoPlot].T, vmin = vmin, vmax = vmax, levels = NN_cont ,cmap='jet')
                cbar = self.figure_Zsing.colorbar(contours, ax=ax)
                cbar.ax.set_ylabel('deltaTrad/<Trad>', rotation=90)
                if contour_check == '1':
                    ax.contour(time_tr,z_tr,data_plot[:,ch_zz,RtoPlot].T, vmin = vmin, vmax = vmax, levels = NN_cont ,cmap='binary')
                ax.set_title("R_ch=%g"%(RtoPlot+1))
                ax.set_ylabel("z [m]")
                ax.set_xlabel("t [s]")
                my_text = ch_zz
                for i in range(len(ch_zz)):
                    ax.annotate(ch_zz[i]+1, (time_tr[i,0], z_tr[i,0]), size=8)


                self.figure_Zsing.suptitle("ECEI, Shot #%d, Filter: %s"%(self.Shot,filter_status), fontsize=10)
    ...
    def CrossCal_ECEI_data(self):
        if (self.data_loaded == True): # check whether ECEI data is loaded
            try:
                ...
                mf=my_funcs.my_funcs()
                mf.Cross_cal_IDA(self.Shot, self.ECEId, self.ECEId_time,self.ECEId_RR,self.ECEId_zz, tCalB, tCalE)
        ...
        if (allow_to_load == True):   
            try:
                ...
                for i, txt in enumerate(my_text):
                    ax.annotate(txt, (EC.R[idx_rztime][i],EC.z[idx_rztime][i] +0.002),fontsize=8)
                ...
                for i, txt in enumerate(my_text_2):
                    ax.annotate(txt, (R_array[i], z_array[i] + 0.002),fontsize=8)

                ax.set_xlabel("R [m]")
                ax.set_ylabel("z [m]")
                ax.legend()

                self.figure_EQHt.suptitle("EQH (rhop), Shot #%d, time = %g s"%(self.Shot_EQH, self.time_EQH), fontsize=12)
                # draw the figure
    ...
    def f_Rz_plot(self, which_plot):
        if (self.data_loaded == True): # check whether ECEI data is loaded
            try:
                ...
                if removeLOS_ch:
                    removeLOS_ch = np.array(self.chzz_ed_RzPl.text().split(','))
                    removeLOS_ch = removeLOS_ch.astype(int) - 1
                else:
                    removeLOS_ch = []
                ...
                check_vmin_vmax = 0 
                if (self.vmin_ed_RzPl.text().replace('-','',1).replace('.','',1).isdigit()==True):
                    vmin = float(self.vmin_ed_RzPl.text())
                    check_vmin_vmax = 1
                else:   
                    vmin = None

                if (self.vmax_ed_RzPl.text().replace('.','',1).isdigit()==True):
                    vmax = float(self.vmax_ed_RzPl.text())
                    check_vmin_vmax = 1
                else:   
                    vmax = None

                if (self.NNcont_ed_RzPl.text().replace('.','',1).isdigit()==True):
                ...
                if (self.Interp_plot_RzPl.currentText() == 'with interpolation'):
                    ...
                    for i_L in range(NN_LOS): 
                        for i_R in range(NN_R):
                            interp_mask[i_L,i_R] = self.checks['%d,%d'%(i_L,i_R)].isChecked()
                ...
                if (self.ImgType_plot_RzPl.currentText() == 'Bilateral'):
                    ...
                    data_plot_t = mf.bilateral_filter(data_plot_t,kernel,kern_size,s0,s1)
                    filter_status += "; Img filt: Bilateral, %s, kern_size=%g, s0=%g, s1=%g"%(kernel,kern_size,s0,s1)
                ...
                if (self.ImgType_plot_RzPl.currentText() == 'Median'):
                    ...
                    data_plot_t = mf.median_filter(data_plot_t,kernel,kern_size)
                    filter_status += "; Img filt: Median, %s, kern_size=%g"%(kernel,kern_size)
                    
                    
                if (self.ImgType_plot_RzPl.currentText() == 'Conservative_smoothing'):
                    size_filt = int(self.ConsSize_ed_RzSet.text())
                    data_plot_t = mf.conservative_smoothing_filter(data_plot_t,size_filt)
                    filter_status += "; Img filt: Conservative smoothing, filt_size=%g"%(size_filt)
                
                
                ### plotting
                # initiate plot 
                self.figure_RzPl.clf() # clear previous figure and axes
                self._static_ax = self.static_canvas_RzPl.figure.subplots(1, 2, sharex=False, sharey=False) # add axes
                if (check_vmin_vmax == 1):
                    levels_to_plot = np.linspace(vmin, vmax, NN_cont)
                if (check_vmin_vmax == 0):
                    levels_to_plot = NN_cont
                contours = self._static_ax[0].contourf(RR_plot, zz_plot, data_plot_t, vmin = vmin, vmax = vmax, levels = levels_to_plot, cmap='jet')
                cbar = self.figure_RzPl.colorbar(contours, ax=self._static_ax[0], pad = 0.07)
                cbar.ax.set_ylabel('deltaTrad/<Trad>', rotation=90)
                if contour_check == '1':
                    self._static_ax[0].contour(RR_plot, zz_plot, data_plot_t, vmin = vmin, vmax = vmax, levels = levels_to_plot, cmap='binary')
                # cbar.ax.tick_params(labelsize=8, rotation=90) 
                self._static_ax[0].plot(RR_plot,zz_plot, "ko", ms = 2)
                
                if (self.Interp_plot_RzPl.currentText() == 'set to zero') | (self.Interp_plot_RzPl.currentText() == 'with interpolation'):
                    self._static_ax[0].plot(RR_plot[interp_mask],zz_plot[interp_mask], "wo", ms = 6)
                ...
                for i, txt in enumerate(ch_zz):
                    self._static_ax[0].annotate(txt+1, (RR_plot[i,0],zz_plot[i,0]),fontsize=8)
                    
                for i, txt in enumerate(ch_RR):
                    self._static_ax[0].annotate(txt+1, (RR_plot[0,i],zz_plot[0,i]),fontsize=8)
                
                self._static_ax[1].plot(time_plot, trace_1D)
                self._static_ax[1].set_xlabel("t [s]")
                self._static_ax[1].set_ylabel("deltaTrad/<Trad>")
                self._static_ax[1].set_title("LOS = 7, R_ch = 4, dt resolut = %g s" %(time_plot[1] - time_plot[0]))
                self._static_ax[1].axvline(x=time_plot_t, color="k")

                self.figure_RzPl.suptitle("ECEI, Shot #%d, Filter: %s"%(self.Shot,filter_status), fontsize=10)
                if (self.Save_plot_RzPl.currentText() == 'save as pdf') | ((self.Save_plot_RzPl.currentText() == 'save as pdf') & (self.counter_save==0)):
                    path_to_save = self.path_ed_RzSet.text()
                    self.figure_RzPl.savefig(path_to_save+'p_%03d.pdf'%(self.counter_save), bbox_inches='tight')
                    self.counter_save += 1
                if (self.Save_plot_RzPl.currentText() == 'save as png') | ((self.Save_plot_RzPl.currentText() == 'save as pdf') & (self.counter_save==0)):
                    path_to_save = self.path_ed_RzSet.text()
                    self.figure_RzPl.savefig(path_to_save+'p_%03d.png'%(self.counter_save), bbox_inches='tight')
                    self.counter_save += 1
                click_coord = self.static_canvas_RzPl.mpl_connect('button_press_event', self.mouse_click_Rz)
